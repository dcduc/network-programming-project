...
class Control(Frame):
    def __init__(
        ...
        self.info.place(x=500, y=680, width=300, height=20)
    ...
    def LiveScreen(self):
        while self.screen_status:
            ...
            while len(image_byte_array) < length_ints:
                buffer = self.screenConnect.recv(BUFSIZ)  # Enc
                if not buffer:
                    break
    ...
    def clickLeft(self, event):
        # Nhấn bên trái
        if (
            event.x >= 0 & event.x <= w - 60 & event.y >= 0 & event.y <= h - 60
            and self.mouse_status
        ):
